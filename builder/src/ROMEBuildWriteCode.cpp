/********************************************************************
  ...
  $Id: ROMEBuildWriteCode.cpp 2631 2012-11-11 16:24:13Z sawada $
...
Bool_t ROMEBuilder::WriteFolderCpp()
{
   ...
   for (int iFold = 0; iFold < numOfFolder; iFold++) {
      ...
      for (i = 0; i < numOfValue[iFold]; i++) {
         if (valueDimension[iFold][i] != 0) {
            if (valueArray[iFold][i][0] == "vector") {
               ...
               buffer.AppendFormatted("   if (fillZero) %s.resize(number, static_cast<%s>(0));\n", valueName[iFold][i].Data(), valueType[iFold][i].Data());
               buffer.AppendFormatted("   else          %s.resize(number);\n", valueName[iFold][i].Data());
               buffer.AppendFormatted("   SetModified(true);\n");
               buffer.AppendFormatted("}\n");
               buffer.AppendFormatted("\n");
            } else if (isFolder(valueType[iFold][i].Data()) ||
      ...
      for (i = 0; i < numOfValue[iFold]; i++) {
          ...
          if (valueArray[iFold][i][0] == "vector") {
            buffer.AppendFormatted("   %s.clear();\n",valueName[iFold][i].Data());
          } else if (isFolder(valueType[iFold][i].Data()) ||
             (valueIsTObject[iFold][i] && valueArray[iFold][i][0] == "variable")) {
            tmp = valueType[iFold][i];
            tmp.ReplaceAll("*","");
      ...
      for (i = 0; i < numOfValue[iFold]; i++) {
         if (valueArray[iFold][i][0] == "vector") {
            buffer.AppendFormatted("   i = 0;\n");
#if 0
            buffer.AppendFormatted("   for (const auto &val: %s) {\n", valueName[iFold][i].Data());
#else
            buffer.AppendFormatted("   for (i = 0; i < static_cast<Int_t>(%s.size()); i++) {\n", valueName[iFold][i].Data());
#endif
            buffer.AppendFormatted("      fieldName = \"%s\";\n", valueName[iFold][i].Data());
            buffer.AppendFormatted("      fieldName += '[';\n");
            buffer.AppendFormatted("      fieldName += i;\n");
            buffer.AppendFormatted("      fieldName += ']';\n");
            buffer.AppendFormatted("      cout<<\"   \"<<setw(tab)<<\"\"<<setw(%d)<<left<<fieldName<<' '<<%s[i]<<'\\n';\n",
         ...
         if (valueDimension[iFold][i] > 0) {
            if (isFolder(valueType[iFold][i].Data())) {
               ...
               buffer.AppendFormatted("Bool_t %s::%sBoundsOk(const char* where, Int_t at) const\n",clsName.Data(),
                                      valueName[iFold][i].Data());
               buffer.AppendFormatted("{\n");
               buffer.AppendFormatted("   return (at < 0 || static_cast<UInt_t>(at) >= %s.size())\n",valueName[iFold][i].Data());
               buffer.AppendFormatted("      ? %sOutOfBoundsError(where, at)\n",valueName[iFold][i].Data());
               buffer.AppendFormatted("      : kTRUE;\n");
               buffer.AppendFormatted("}\n");
               buffer.AppendFormatted("\n");
...
Bool_t ROMEBuilder::WriteFolderH()
{
   ...
   for (int iFold = 0; iFold < numOfFolder; iFold++) {
      ...
      for (i = 0; i < numOfValue[iFold]; i++) {
         if ((isFolder(valueType[iFold][i].Data()) && valueDimension[iFold][i] > 0) ||
             (valueIsTObject[iFold][i] && valueArray[iFold][i][0] == "variable") ||
             (valueIsTObject[iFold][i] && valueArray[iFold][i][0] == "vector")) {
...
Bool_t ROMEBuilder::WriteBaseTabCpp()
{
   ...
   for (iTab = 0; iTab < numOfTab; iTab++) {
      ...
      if (numOfTabSingleObjects[iTab] > 0) {
         ...
         for (i = 0; i < numOfTabSingleObjects[iTab]; i++) {
            if (tabSingleObjectType[iTab][i] == "Histogram") {
               ...
               } else if (tabSingleObjectDrawSamePad[iTab][i]) {
      ...
      second.resize(tabSingleObjectIndexMax[iTab], false);
      if (numOfTabSingleObjects[iTab] > 0) {
                  ...
                  if (tabSingleObjectType[iTab][j] == "Histogram") {
                     if (histoArraySize[tabSingleObjectTaskIndex[iTab][j]][tabSingleObjectObjectIndex[iTab][j]] == "1") {
                        if (tabSingleObjectIndex[iTab][j] != -1) {
                           padNumber = tabSingleObjectIndex[iTab][j] + 1;
                        } else {
                           // automatical order
                           padNumber = i + 1;
                        }
                        ...
                        second[padNumber] = true;
                     } else if (tabSingleObjectDrawSamePad[iTab][j]) {
                        if (tabSingleObjectIndex[iTab][j] != -1) {
                           padNumber = tabSingleObjectIndex[iTab][j] + 1;
                        } else {
                           // automatical order
                        ...
                        for (k = tabSingleObjectArrayIndexStart[iTab][j]; k <= tabSingleObjectArrayIndexEnd[iTab][j];
                             k++) {
                           if (tabSingleObjectIndex[iTab][j] != -1) {
                              padNumber = tabSingleObjectIndex[iTab][j] + 1 + k - tabSingleObjectArrayIndexStart[iTab][j];
                           } else {
                              // automatical order
                              padNumber = i + 1 + k - tabSingleObjectArrayIndexStart[iTab][j];
                           }
                           buffer.AppendFormatted("      fGeneratedCanvas->GetCanvas()->cd(%d);\n", padNumber);
         ...
         for (i = 0; i < numOfTabObjectDisplayObjectTypes[iTab]; i++) {
            ...
            if (tabObjectDisplayObjectType[iTab][i] == "ROMETGraph" ||
               ...
               buffer.AppendFormatted("      static_cast<TObjArray*>(fObjects->Last())->AddLast(new %s(str.Data(),\"\", 1, 0, 1, 1, 0, 1, 1, 0, 1));\n",
                                      tabObjectDisplayObjectType[iTab][i].Data());
            } else if (tabObjectDisplayObjectType[iTab][i] == "TProfile") {
               buffer.AppendFormatted("      static_cast<TObjArray*>(fObjects->Last())->AddLast(new %s(str.Data(),\"\", 1, 0, 1));\n",
                                      tabObjectDisplayObjectType[iTab][i].Data());
            }
...
//         buffer.AppendFormatted("   fNetFolderRoot[%d]   = \"histos\";\n",i);
   ...
   buffer.AppendFormatted("{\n");
   buffer.AppendFormatted("   f%sConfigParametersFolder->Write(\"%sConfigParameters\", fOutputObjOption);\n",
...
Bool_t ROMEBuilder::WriteEventLoopCpp()
{
   ...
   for (i = 0; i < numOfTree; i++) {
      ...
      for (j = 0; j < numOfRunHeader[i]; j++) {
         if (folderUsed[runHeaderFolderIndex[i][j]]) {
            if (folderArray[runHeaderFolderIndex[i][j]] == "1") {
               buffer.AppendFormatted("            gAnalyzer->Get%s()->Write(\"%s\", gAnalyzer->GetOutputObjOption());\n",
                                      runHeaderFolder[i][j].Data(), runHeaderName[i][j].Data());
            } else {
               buffer.AppendFormatted("            gAnalyzer->Get%ss()->Write(\"%s\", gAnalyzer->GetOutputObjOption() |  TObject::kSingleKey);\n",
                                      runHeaderFolder[i][j].Data(), runHeaderName[i][j].Data());
            }
         }
      }
...
#if (ROOT_VERSION_CODE >= ROOT_VERSION(5,30,0))
   buffer.AppendFormatted("   if (ROOT_VERSION(rootMajor, rootMinor, 0) != (ROOT_VERSION_CODE & 0xFFFFFF00)) {\n");
#else
...
//   buffer.AppendFormatted("#include <TSystem.h>\n");
   ...
   for (iTree = 0; iTree < numOfTree; iTree++) {
      buffer.AppendFormatted(",\n                  Bool_t read%s = kTRUE", treeName[iTree].Data());
   }
   buffer.AppendFormatted("\n");
   buffer.AppendFormatted("                 )\n");
   buffer.AppendFormatted("{\n");
...
//______________________________________________________________________________
Bool_t ROMEBuilder::WriteDistillTreesC()
{
   // Write sample macro to read output trees
   int i;
   int iFold;
   ...
   for (iTree = 0; iTree < numOfTree; iTree++) {
      ...
      WriteHeader(buffer, numOfMainAuthors, mainAuthor, mainEmail, true);
      macroDescription.AppendFormatted("This macro extracts a part of data and makes smaller trees.\n\n");
      macroDescription.AppendFormatted(" Usage\n");
      ...
      macroDescription.AppendFormatted("   %s%s [1] %sDistill%sTree(\"./\", \"./\",\n",
                                       shortCut.ToLower(tmp), mainProgName.ToLower(tmp2), shortCut.Data(), treeName[iTree].Data());
...
//   buffer.AppendFormatted("#include <TSystem.h>\n");
      ...
      buffer.AppendFormatted("#   include \"include/generated/%sAllFolders.h\"\n",shortCut.Data());
      buffer.AppendFormatted("#else\n");
      buffer.AppendFormatted("class ROMETreeInfo;\n");
      for (iBranch = 0; iBranch < numOfBranch[iTree]; iBranch++) {
         if (branchFolderNum[iTree][iBranch] == -1) {
            continue;
         }
      ...
      buffer.AppendFormatted("// Compression level of output tree\n");
      buffer.AppendFormatted("const Bool_t kCompression = 1;\n");
      buffer.AppendFormatted("\n");
      ...
      buffer.AppendFormatted("\n");
      buffer.AppendFormatted("Bool_t EventSelection() {\n");
      buffer.AppendFormatted("   // This is a place holder of a function for event selection. This function is\n");
      buffer.AppendFormatted("   // called at every events between reading input file and wring output file.\n");
      buffer.AppendFormatted("   // When it returns kTRUE, an event is written to output file. When it returns\n");
      buffer.AppendFormatted("   // kFALSE, an event is not written to output file. It is also possible to\n");
      buffer.AppendFormatted("   // modify data. Global variables defined above are available.\n");
      buffer.AppendFormatted("\n");
      buffer.AppendFormatted("   return kTRUE;\n");
      buffer.AppendFormatted("}\n");
      buffer.AppendFormatted("\n");
      ...
      // Open files
      buffer.AppendFormatted("   TString  inFileName = indir;\n");
      buffer.AppendFormatted("   TFile   *inFile = 0;\n");
      buffer.AppendFormatted("   TString  outFileName = outdir;\n");
      buffer.AppendFormatted("   TFile   *outFile = 0;\n");
      buffer.AppendFormatted("   TString  runNumberString;\n");
      buffer.AppendFormatted("   runNumberString.Form(\"%%05d\", run);\n");
      buffer.AppendFormatted("\n");

      buffer.AppendFormatted("   if (!inFileName.Length()) {\n");
      buffer.AppendFormatted("      inFileName = \".\";\n");
      buffer.AppendFormatted("   }\n");
      buffer.AppendFormatted("   if (!inFileName.EndsWith(\"/\")) {\n");
      buffer.AppendFormatted("      inFileName += \"/\";\n");
      buffer.AppendFormatted("   }\n");
      buffer.AppendFormatted("   if (!infile || infile[0] == '\\0') {\n");
      buffer.AppendFormatted("      inFileName += \"%s#.root\";\n", treeName[iTree].Data());
      buffer.AppendFormatted("   } else {\n");
      buffer.AppendFormatted("      inFileName += infile;\n");
      buffer.AppendFormatted("   }\n");
      buffer.AppendFormatted("   inFileName.ReplaceAll(\"#\", runNumberString.Data());\n");
      buffer.AppendFormatted("\n");

      buffer.AppendFormatted("   if (!outFileName.Length()) {\n");
      buffer.AppendFormatted("      outFileName = \".\";\n");
      buffer.AppendFormatted("   }\n");
      buffer.AppendFormatted("   if (!outFileName.EndsWith(\"/\")) {\n");
      buffer.AppendFormatted("      outFileName += \"/\";\n");
      buffer.AppendFormatted("   }\n");
      buffer.AppendFormatted("   if (!outfile || outfile[0] == '\\0') {\n");
      buffer.AppendFormatted("      outFileName += \"%s#_new.root\";\n", treeName[iTree].Data());
      buffer.AppendFormatted("   } else {\n");
      buffer.AppendFormatted("      outFileName += outfile;\n");
      buffer.AppendFormatted("   }\n");
...
Bool_t ROMEBuilder::WriteVersionH()
{
   ...
   if (gSystem->AccessPathName(path,kFileExists)) {
      ...
      cmd.SetFormatted("svn info %s --xml | grep wcroot-abspath | cut -d \\> -f 2 | cut -d \\< -f 1 | head -n 1", outDir.Data());
...
#if defined( R__UNIX )
#if 1 /* using svn info */
         cmd.SetFormatted("svn info %s --xml | grep revision | cut -d \\\" -f 2 | head -n 1", outDir.Data());
   ...
   ROMEString gitDescription = "";

   // Read Git description
   ROMEString gitDesc;
   cmd.SetFormatted("cd %s; git describe --dirty --tags", outDir.Data());
   gitDesc.ReadCommandOutput(cmd.Data() , false, true);
...
//   const char* const monthName[] = {"Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"};
   ...
   buffer.AppendFormatted("#define %s_GIT_DESCRIPTION   \"%s\"\n",shortCut.ToUpper(tmp),gitDesc.Data());
#if 0
   ...
   if (!romeStable) {
      buffer.AppendFormatted("      \"*   %-36s*\\n\"\n", romeDescription.Data());
   } else {
      buffer.AppendFormatted("      \"*             %-14s            *\\n\"\n",romeVersion.Data());
   }
   buffer.AppendFormatted("      \"*                                       *\\n\"\n");
   buffer.AppendFormatted("      \"*****************************************\\n\\n\";\n");
   buffer.AppendFormatted("\n");
