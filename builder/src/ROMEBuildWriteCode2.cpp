/********************************************************************
  ...
  $Id: ROMEBuildWriteCode2.cpp 2633 2012-12-20 16:31:06Z sawada $
...
Bool_t ROMEBuilder::AddConfigParameters()
{
   ...
   // Common
   {
      ...
      // Settings
      {
         ...
         subSubGroup->AddParameter(new ROMEConfigParameter("ReportSummaryFileName"));
         subSubGroup->GetLastParameter()->ReadComment(ROMEConfig::kCommentLevelParam, subSubGroup->GetGroupName());
         subSubGroup->GetLastParameter()->AddSetLine("gAnalyzer->SetReportSummaryFileName(##);");
         subSubGroup->GetLastParameter()->AddWriteLine("writeString = gAnalyzer->GetReportSummaryFileName();");
         // ReportSummaryFileLevel
         subSubGroup->AddParameter(new ROMEConfigParameter("ReportSummaryFileLevel","1","ComboBox"));
         subSubGroup->GetLastParameter()->ReadComment(ROMEConfig::kCommentLevelParam, subSubGroup->GetGroupName());
         subSubGroup->GetLastParameter()->AddSetLine("inttmp = strtol(##,&cstop, 10);");
         subSubGroup->GetLastParameter()->AddSetLine("if (## == \"mute\" || ((!cstop || !strlen(cstop)) && inttmp == ROMEPrint::kMute)) {");
         subSubGroup->GetLastParameter()->AddSetLine("   gAnalyzer->SetReportSummaryFileLevel(ROMEPrint::kMute);");
         subSubGroup->GetLastParameter()->AddSetLine("} else if (## == \"error\" || ((!cstop || !strlen(cstop)) && inttmp == ROMEPrint::kError)) {");
         subSubGroup->GetLastParameter()->AddSetLine("   gAnalyzer->SetReportSummaryFileLevel(ROMEPrint::kError);");
         subSubGroup->GetLastParameter()->AddSetLine("} else if (## == \"warning\" || ((!cstop || !strlen(cstop)) && inttmp == ROMEPrint::kWarning)) {");
         subSubGroup->GetLastParameter()->AddSetLine("   gAnalyzer->SetReportSummaryFileLevel(ROMEPrint::kWarning);");
         subSubGroup->GetLastParameter()->AddSetLine("} else if (## == \"normal\" || ((!cstop || !strlen(cstop)) && inttmp == ROMEPrint::kNormal)) {");
         subSubGroup->GetLastParameter()->AddSetLine("   gAnalyzer->SetReportSummaryFileLevel(ROMEPrint::kNormal);");
         subSubGroup->GetLastParameter()->AddSetLine("} else if (## == \"verbose\" || ((!cstop || !strlen(cstop)) && inttmp == ROMEPrint::kVerbose)) {");
         subSubGroup->GetLastParameter()->AddSetLine("   gAnalyzer->SetReportSummaryFileLevel(ROMEPrint::kVerbose);");
         subSubGroup->GetLastParameter()->AddSetLine("} else if (## == \"debug\" || ((!cstop || !strlen(cstop)) && inttmp == ROMEPrint::kDebug)) {");
         subSubGroup->GetLastParameter()->AddSetLine("   gAnalyzer->SetReportSummaryFileLevel(ROMEPrint::kDebug);");
         subSubGroup->GetLastParameter()->AddSetLine("}");
         subSubGroup->GetLastParameter()->AddWriteLine("if (gAnalyzer->GetReportSummaryFileLevel() == ROMEPrint::kMute) {");
         subSubGroup->GetLastParameter()->AddWriteLine("   writeString = \"mute\";");
         subSubGroup->GetLastParameter()->AddWriteLine("} else if (gAnalyzer->GetReportSummaryFileLevel() == ROMEPrint::kError) {");
         subSubGroup->GetLastParameter()->AddWriteLine("   writeString = \"error\";");
         subSubGroup->GetLastParameter()->AddWriteLine("} else if (gAnalyzer->GetReportSummaryFileLevel() == ROMEPrint::kWarning) {");
         subSubGroup->GetLastParameter()->AddWriteLine("   writeString = \"warning\";");
         subSubGroup->GetLastParameter()->AddWriteLine("} else if (gAnalyzer->GetReportSummaryFileLevel() == ROMEPrint::kNormal) {");
         subSubGroup->GetLastParameter()->AddWriteLine("   writeString = \"normal\";");
         subSubGroup->GetLastParameter()->AddWriteLine("} else if (gAnalyzer->GetReportSummaryFileLevel() == ROMEPrint::kVerbose) {");
         subSubGroup->GetLastParameter()->AddWriteLine("   writeString = \"verbose\";");
         subSubGroup->GetLastParameter()->AddWriteLine("} else if (gAnalyzer->GetReportSummaryFileLevel() == ROMEPrint::kDebug) {");
         subSubGroup->GetLastParameter()->AddWriteLine("   writeString = \"debug\";");
         subSubGroup->GetLastParameter()->AddWriteLine("} else {");
         subSubGroup->GetLastParameter()->AddWriteLine("   writeString = \"normal\";");
         subSubGroup->GetLastParameter()->AddWriteLine("}");
...
#   endif
            ...
            subSubSubGroup->GetLastParameter()->AddSetLine("}");
            subSubSubGroup->GetLastParameter()->AddWriteLine("if (!gAnalyzer->IsROMEMonitor()) {");
            subSubSubGroup->GetLastParameter()->AddWriteLine("   if (gAnalyzer->GetTreeObjectAt(%d)->GetCompressionAlgorithm() == ROOT::kLZMA) {", i);
            subSubSubGroup->GetLastParameter()->AddWriteLine("      writeString = \"LZMA\";");
#   if (ROOT_VERSION_CODE >= ROOT_VERSION(6,19,0))            
            subSubSubGroup->GetLastParameter()->AddWriteLine("   } else if (gAnalyzer->GetTreeObjectAt(%d)->GetCompressionAlgorithm() == ROOT::kZSTD) {", i);
            subSubSubGroup->GetLastParameter()->AddWriteLine("      writeString = \"ZSTD\";");
#   endif
            subSubSubGroup->GetLastParameter()->AddWriteLine("   } else {");
            subSubSubGroup->GetLastParameter()->AddWriteLine("      writeString = \"ZLIB\";");
            subSubSubGroup->GetLastParameter()->AddWriteLine("   }");
            subSubSubGroup->GetLastParameter()->AddWriteLine("}");
#endif
            // AutoSaveSize
